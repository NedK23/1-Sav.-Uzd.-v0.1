...
void Studentas::OutputToUserFile(const std::string& fileName, const std::vector<Studentas>& studentai) {
...
void Studentas::GeneruotiStudenta(Studentas &studentas) {
  ...
  for (int i = 0; i < namuDarbuSkaicius; ++i) {
    studentas.GetNamuDarbai()[i] = N(gen) + 1;
  }
  ...
  totalTimeSum += sortingTime.count();
